...
export async function GET() {
  try {
    ...
    if (!response.ok) {
      return NextResponse.json({ lastPlayed: null, error: "Failed to fetch last played track" }, { status: response.status });
    }
    ...
    return NextResponse.json({ lastPlayed: null, error: "Failed to fetch last played" }, { status: 500 });
  }
}
